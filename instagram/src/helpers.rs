...
pub fn set_photo_taken_time(photo_file_path:&String,timestamp:i64)->Result<(), Error>{
    ...
    return set_file_times(photo_path, time, time, time);

}

pub fn get_directories(src_path:&str,directory_path: &str)-> Result<Vec<String>, std::io::Error>{
...
pub fn get_json_file_names(src_path:&str,directory_path: &str,prefix_match:&str) -> Result<Vec<String>, std::io::Error> {
    let full_path: String = format!("{}/{}",src_path,directory_path);
    ...
    .filter_map(|entry| {
        entry.ok().and_then(|e| {
            ...
            if path.is_file()
                ...
                && path.file_stem().map_or(false, |stem| stem.to_string_lossy().starts_with(prefix_match))
            {
